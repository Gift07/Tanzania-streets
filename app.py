...
@app.route('/')
def show_entries():
    regions = ['arusha', 'daressalaam', 'dodoma', 'geita', 'iringa', 'kagera', 'katavi', 'kigoma', 'kilimanjaro', 'lindi', 'manyara', 'mara',
               'mbeya', 'morogoro', 'mtwara', 'mwanza', 'njombe', 'pwani', 'rukwa', 'ruvuma', 'shinyanga', 'simiyu', 'singida', 'singida', 'songwe', 'tabora']
    available_regions = {"regions": regions}
    return json.dumps(available_regions)
    ...
    if region_name in regions:
        ...
        if district_name in districts:
            ...
            if ward_name in wards:
                ...
                ward = wards[ind]
                sql_code = "SELECT DISTINCT street from " + \
                    region + ' where district = ? and ward = ?'
